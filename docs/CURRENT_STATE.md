...
**Current Phase**: Phase 4, Literature Basis v1, Model Decision Basis v1, Phase 5 Gate A, Gate B, & Gate C **FROZEN**; Phase 5 Gate D (Reconstruct Published Material Inference Logic) **ACTIVE NEXT GATE**
...
- **Phase 5 Gate C (Image Semantics & Reconstructed Intensity Characterization)**: **VERIFIED & FROZEN (AMENDED)** ([`reports/phase5_gate_c_semantics_report.md`](../reports/phase5_gate_c_semantics_report.md); 100% histogram accounting across $396,857,344$ voxels, full-volume dynamic range $[0, 65535]$ with mean $10,607.0$, objective Otsu threshold $20,864$; establishes that CT intensity alone does not recover the hypothesized Zone 2/Zone 3 boundary in sampled dome regions: among bone voxels, $\text{CNR} = 0.0616 \ll 1.0$, $D_B = 0.0134$, descriptive $\text{ROC AUC} = 0.5132$ between dorsal cortex Zone 3 and cancellous core Zone 2, stable across $T \in [15000, 25000]$; unfiltered core ROI contains $16.2\%$ low-intensity voxels ($11.8\% < 10,000$), aligning with published observations of spatial trabecular heterogeneity (Snively & Theodor 2011); whole-volume bone mask exhibits bimodal peaks ($34,042$ bone vs $41,189$ rock matrix); residual radial intensity drop $11.34\%$; Decision [`D011`](DECISIONS.md) mandating that Model B histological zonation be constructed via literature-informed geometric rules from thin-section histology [Schott et al. 2011, Snively & Theodor 2011] rather than direct CT thresholding; metrics in [`results/phase5/gate_c_semantics_metrics.json`](../results/phase5/gate_c_semantics_metrics.json); Figures 13 & 14 in [`reports/figures/`](../reports/figures/)).
- **Immediate Next Action (Gate D)**: Reconstruct published material inference logic (Decision Basis v1 §4.4, Decision D012). Formalize explicit mathematical and spatial rules from Snively & Theodor (2011) and Schott et al. (2011) into executable code to define the 3-zone material assignment for Model B.
