...
- Core dependencies defined in [`pyproject.toml`](../pyproject.toml): `numpy`, `scipy`, `matplotlib`, `trimesh`, `pyvista`, `pytest`
- Lockfile: `uv.lock`

### Execution
1. Solve all 3 production tetrahedral tiers sequentially in isolated Python processes:
   ```bash
   uv run python -m stegoceras_biomechanics.fea.solve_production --tier all
   ```
   *(Alternatively, solve individual tiers: `--tier coarse`, `--tier medium_coarse`, `--tier medium`)*
2. Primary solver entry points:
   - CLI / process manager: [`src/stegoceras_biomechanics/fea/solve_production.py`](../src/stegoceras_biomechanics/fea/solve_production.py) (`run_all_tiers_isolated()` spawning subprocesses calling `solve_tier()`)
   - Mathematical solver: [`src/stegoceras_biomechanics/fea/solver.py`](../src/stegoceras_biomechanics/fea/solver.py) (`solve_linear_elasticity()`)
   - Boundary surface extraction: [`src/stegoceras_biomechanics/fea/meshing.py`](../src/stegoceras_biomechanics/fea/meshing.py) (`extract_boundary_surface()`)
   - Dome load formulation: [`src/stegoceras_biomechanics/fea/loads.py`](../src/stegoceras_biomechanics/fea/loads.py) (`generate_dome_load_patch()`)
   - Constraint generation: [`src/stegoceras_biomechanics/fea/boundary_conditions.py`](../src/stegoceras_biomechanics/fea/boundary_conditions.py) (`generate_boundary_constraints()`)
   - Subregion metrics extraction: [`src/stegoceras_biomechanics/fea/results.py`](../src/stegoceras_biomechanics/fea/results.py) (`extract_subregion_metrics()`)

### Post-processing / Analysis
1. Post-processing and derived numerical analysis command:
   ```bash
   uv run python -m stegoceras_biomechanics.fea.plot_results
   ```
2. Analysis entry point:
   - Module: [`src/stegoceras_biomechanics/fea/plot_results.py`](../src/stegoceras_biomechanics/fea/plot_results.py)
   - Function: `generate_all_phase4_results_and_plots()`
3. Scientific analysis performed during post-processing:
   - Ingests raw simulation artifacts (`simulations/phase4/solution_{tier}.npz`) and mesh geometries (`data/meshes/cleaned/stegoceras_tetmesh_{tier}.npz`).
   - Extracts tetrahedral aspect ratio distributions, computing 50th percentile, 95th percentile, max, and mean aspect ratios.
   - Evaluates apex node displacement for each mesh tier.
   - Computes global 95th and 99th percentile von Mises stress for all tiers.
   - Extracts regional stresses (dome apex and endocranial braincase roof) from raw subregion metrics.
   - Derives the 3-point cross-tier convergence summary and writes the authoritative derived comparison artifact:
     [`results/phase4/mesh_convergence_comparison.json`](../results/phase4/mesh_convergence_comparison.json).

### Figure generation
Generated by `uv run python -m stegoceras_biomechanics.fea.plot_results`:
- Figure 08: [`reports/figures/08_mesh_resolutions_comparison.png`](figures/08_mesh_resolutions_comparison.png) (Element volume & aspect ratio distributions)
- Figure 09: [`reports/figures/09_fe_von_mises_stress_1kn.png`](figures/09_fe_von_mises_stress_1kn.png) (Sagittal stress field & regional bar chart)
- Figure 10: [`reports/figures/10_fe_displacement_and_strain.png`](figures/10_fe_displacement_and_strain.png) (Displacement & principal tensile strain fields)
- Figure 11: [`reports/figures/11_mesh_convergence_curves.png`](figures/11_mesh_convergence_curves.png) (Strain energy and regional stress convergence trajectories)
- Figure 12: [`reports/figures/12_linearity_scaling_validation.png`](figures/12_linearity_scaling_validation.png) (Constitutive linearity and quadratic energy scaling)
...
- Raw regional and summary metrics:
  ...
  - `results/phase4/ualvp2_1kn_subregion_metrics.{json,csv}` (canonical primary benchmark)
  - `results/phase4/summary_{coarse,medium_coarse,medium}.json`
- Derived analysis artifact:
  - [`results/phase4/mesh_convergence_comparison.json`](../results/phase4/mesh_convergence_comparison.json)
- Rendered figures:
  - `reports/figures/08_mesh_resolutions_comparison.png`
  - `reports/figures/09_fe_von_mises_stress_1kn.png`
  - `reports/figures/10_fe_displacement_and_strain.png`
  - `reports/figures/11_mesh_convergence_curves.png`
  - `reports/figures/12_linearity_scaling_validation.png`

### Report provenance
- **Governing Design**: [`docs/phase_design/PHASE4_DESIGN_RECONSTRUCTED.md`](../docs/phase_design/PHASE4_DESIGN_RECONSTRUCTED.md) *(Retrospective Reconstruction)*
...
- **Verification tests**: `uv run pytest tests/test_phase4_fea.py -v` (53 tests verifying analytical solution, manufactured displacement field, work-energy identity, and convergence rates)
- **Governing decisions**: Decisions `D003`, `D004`, `D005`, `D006` in [`docs/DECISIONS.md`](../docs/DECISIONS.md); Phase 4 Freeze in [`docs/CURRENT_STATE.md`](../docs/CURRENT_STATE.md) and [`HANDOFF.md`](../HANDOFF.md)
