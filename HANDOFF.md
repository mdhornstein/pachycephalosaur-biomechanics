...
**Current Phase**: Phase 4, Literature Basis v1, Phase 5 Gate A, Gate B, & Gate C **FROZEN**; Phase 5 Gate D (Reconstruct Published Material Inference Logic) **ACTIVE NEXT GATE**  
...
3. **Gate C — Characterize Image Data Semantics**: **VERIFIED & FROZEN** ([`reports/phase5_gate_c_semantics_report.md`](reports/phase5_gate_c_semantics_report.md); 100% histogram accounting across $396,857,344$ voxels, full-volume dynamic range $[0, 65535]$ with mean $10,607.0$, objective Otsu threshold $20,864$; refutes discrete radiological zonation and confirms diagenetically permineralized uniform dome profile: $\text{CNR} = 0.0616 \ll 1.0$, $D_B = 0.0134$, $\text{ROC AUC} = 0.5132$ between dorsal cortex Zone 3 and cancellous core Zone 2; beam-hardening cupping drop $11.34\%$; Decision [`D011`](docs/DECISIONS.md) mandating that Model B histological zonation be constructed via literature-informed geometric rules from thin-section histology [Schott et al. 2011, Snively & Theodor 2011] rather than direct CT thresholding; metrics in [`results/phase5/gate_c_semantics_metrics.json`](results/phase5/gate_c_semantics_metrics.json); Figures 13 & 14 in [`reports/figures/`](reports/figures/)).
4. **Gate D — Reconstruct Published Material Inference Logic (ACTIVE NEXT GATE)**: Document what Snively & Theodor (2011) directly observed from CT vs. what was assumed or thresholded, and formalize explicit mathematical and spatial rules for 3-zone material assignment.
