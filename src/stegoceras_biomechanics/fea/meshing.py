...
def generate_tetrahedral_mesh(
    ...
    """Generates a solid 3D tetrahedral FE mesh from a watertight surface.
    
    Supports resolution tiers:
    - 'coarse': Decimated surface base (~100k-200k tets) for rapid verification.
    - 'medium': Moderately refined surface (~400k-800k tets) for stable benchmark solve.
    - 'fine': Full surface resolution (~1.5M-2.3M tets) for high-resolution convergence check.
    """
    ...
    if resolution_tier.lower() == "coarse":
        pv_surf = pv.PolyData(v, np.c_[np.full(len(f), 3), f])
        pv_dec = pv_surf.decimate(0.96)
    ...
    elif resolution_tier.lower() == "medium":
        pv_surf = pv.PolyData(v, np.c_[np.full(len(f), 3), f])
        pv_dec = pv_surf.decimate(0.85)
        f_dec = pv_dec.faces.reshape(-1, 4)[:, 1:4]
        v_dec = pv_dec.points
